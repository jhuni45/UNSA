...
float phi= 0.99, alfa = 1.0, beta = 1.0, Q=1.0;
...
void printCities(vec_num  &vec, int n){
      int city;
...
void iniMatrices(){
      ...
      vector<float> b(n_ciudades*n_ciudades,INITIAL_FERO);
...
void show_Poblation(){
      ...
      for (size_t i = 0; i < poblation.size(); i++){
            file<<"Hormiga "<<i<<": "<<poblation[i].ruta<<" - Costo: "<<poblation[i].l<<" dis: ";
            show_route(poblation[i]);
      }
      file<<"------------------------------------------------\n";
}

int IndVector(vector<city> &vec, int x){   
      auto it = find(vec.begin(), vec.end(), x);
      if(it != vec.end())
            return  distance(vec.begin(), it);
}

float buscar_ij(ant &tmp, int pos, int j){
...
float nivelFeromona(int i, int j){
      ...
      for(size_t it=0; it<poblation.size(); it++){
            pos = IndVector(poblation[it].cities,i);
            aux = buscar_ij(poblation[it],pos,j);
            t_ij += aux;
            if (aux<0.0000000001)
                  file <<"  +  0";
            else
...
void actualizarMatrizFeronoma(){
      for (size_t i = 0; i < n_ciudades; i++){
            for (size_t j = 0; j < n_ciudades; j++){
                  if(j != i){
                        file<<s_ciudades[i]<<"  -  "<<s_ciudades [j]<<" => "<<phi*Mf[i*n_ciudades+j];
                        float aux=nivelFeromona(i,j);
                        Mf[i*n_ciudades+j] = phi*Mf[i*n_ciudades+j]+aux;
                        file<<" => "<<Mf[i*n_ciudades+j]<<endl;
                  }
            } 
      }
}
